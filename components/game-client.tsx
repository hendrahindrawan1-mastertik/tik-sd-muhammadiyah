...
export function GameClient({ soalList }: { soalList: Soal[] }) {
  ...
  const mulai = () => {
    setSoalAcak(acakSoal(soalList, Math.min(20, soalList.length)))
    setNomor(0)
    setSkor(0)
    ...
    return (
      <div className="mx-auto max-w-sm rounded-3xl border border-border bg-card p-6 text-center">
        ...
        <p className="mb-5 text-sm text-brand-muted">
          {Math.min(20, soalList.length)} soal, {DETIK_PER_SOAL} detik per soal. Jawab secepat mungkin!
        </p>
        <button
          onClick={mulai}
